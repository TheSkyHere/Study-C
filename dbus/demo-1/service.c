...
DBusConnection* init_bus()
{
    ...
    /*
    busctl tree --user  //查看用户dbus
    busctl tree --system  //查看系统dbu
     */
    ...
    //                   }
    //param 4: err info
    ret = dbus_bus_request_name(connection, "hello.world.Unity", DBUS_NAME_FLAG_REPLACE_EXISTING, &err);
    if(dbus_error_is_set(&err))
    ...
    dbus_bus_add_match(connection, "interface='aa.bb.cc'", &err);  // 定义匹配器  
    //阻塞，直到消息发送成功．
