...
static gboolean send_ding(DBusGConnection *bus)  // 定义发送消息函数的细节  
{  
   DBusGMessage *message;                        // 创建消息对象指针  
   message = dbus_message_new_signal("/com/burtonini/dbus/ding",   
                                       "com.burtonini.dbus.Signal",  
                                       "ding");     // 创建消息对象并标识路径  
   dbus_message_append_args(message,  
                            G_TYPE_STRING, "ding!",  
                            G_TYPE_INVALID);     //将字符串Ding!定义为消息  
   dbus_g_connection_open(bus, message, NULL);    // 发送该消息  
   dbus_g_message_unref(message);                 // 释放消息对象  
